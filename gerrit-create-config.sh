...
set_gerrit_config() {
    su-exec ${GERRIT_USER} git config -f "${GERRIT_DIR_VOLUME}/etc/gerrit.config" "$@"
}

set_secure_config() {
    su-exec ${GERRIT_USER} git config -f "${GERRIT_DIR_VOLUME}/etc/secure.config" "$@"
}

set_replication_config() {
    su-exec ${GERRIT_USER} git config -f "${GERRIT_DIR_VOLUME}/etc/replication.config" "$@"
}
...
if [ -n "${REPLICATION_REMOTES}" ]; then
    ...
    for r in ${REPLICATION_REMOTES}; do
        ...
        if ! $(git config -f "${GERRIT_DIR_VOLUME}/etc/replication.config" --get-all remote.${r}.projects > /dev/null); then
            for p in ${PROJECTS}; do
            set_replication_config --add remote.${r}.projects "${p}"
            done
        fi

        if ! $(git config -f "${GERRIT_DIR_VOLUME}/etc/replication.config" --get-all remote.${r}.push > /dev/null); then
    ...
    done # for r in ${REPLICATION_REMOTES}
fi # if [ -n "${REPLICATION_REMOTES}" ]
...
if [ "${AUTH_TYPE}" = 'OAUTH' ]  ; then
    su-exec ${GERRIT_USER} cp -f ${GERRIT_HOME}/oauth.jar ${GERRIT_DIR_VOLUME}/plugins/oauth.jar
...
#set_gerrit_config plugin.events-log.storeUrl ${GERRIT_EVENTS_LOG_STOREURL:-"jdbc:h2:${GERRIT_DIR_VOLUME}/db/ChangeEvents"} # TODO. EVENT
